...
library(sysfonts)
library(showtext)
sysfonts::font_add("Lora", system.file("fonts/Lora-Regular.ttf", package = "eNFI", mustWork = T))
sysfonts::font_add("LoraIt", system.file("fonts/Lora-Italic.ttf", package = "eNFI", mustWork = T))
sysfonts::font_add("Shadow", system.file("fonts/ShadowsIntoLight-Regular.ttf", package = "eNFI", mustWork = T))
...
<img src="images/newland.gif" width="250" height="250" alt="eNFI lesson 1 image cover" align="right" style="margin: -1em 0 -1em 0"/>
...
Here is what ```r newland_name```'s land covers look like.
...
```{r fi-agb, exercise=TRUE, exercise.lines=8}
exfi_agb2 <- exfi_pagb %>%
  summarise(
    mean_agb = mean(___),
    sd_agb   = sd(___)
...
```{r lc-area-recalc, include=FALSE}
area_tot <- sf_lc %>%
  mutate(
    area_m2 = st_area(.),
    area_ha = units::set_units(area_m2, value = ha)
...
We have found that the total area of ```r newland_name``` is ```r round(area_tot$area_ha)``` ha. Out of it, ```r round(area_lc %>% filter(lc == "WA") %>% pull(area_ha))``` and ```r round(area_lc %>% filter(lc == "NF") %>% pull(area_ha))``` ha are Water and Non-forest respectively, leaving ```r round(area_lc %>% filter(!(lc %in% c("WA", "NF"))) %>% pull(area_ha) %>% sum())```
 ha of Forest land.
